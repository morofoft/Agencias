// js/dataAgencias.js - VERSIÓN CORREGIDA

import { dbPromise } from './db/db.js';
import { getAllAgencies } from './agencies/agencies.store.js'; // ← AGREGAR
...
function filterByState(list, state) {
    if (state === 'located') return list.filter(a => a.lat !== null);
    if (state === 'pending') return list.filter(a => a.lat === null);
    return list;
}
...
// ✅ DESCOMENTAR Y CORREGIR
function renderAgencies(agencies) {
    if (!list) return;
    list.innerHTML = '';

    if (!agencies.length) {
        list.innerHTML = `<div class="text-center py-20 bg-white rounded-[2rem] border-2 border-dashed border-slate-200/60">
            ...
            <div class="flex items-center gap-5">
                ...
                <div class="space-y-1">
                    <div class="flex flex-wrap items-center gap-3">
                        <h3 class="text-lg font-bold text-slate-800 tracking-tight">${escapeHtml(a.idReal)}</h3>
                        ${statusBadge}
                    </div>
                    <div class="flex flex-col">
                        <p class="text-[10px] font-black text-indigo-500 uppercase tracking-widest mb-1">Zona ${escapeHtml(a.zona)}</p>
                        <p class="text-xs font-medium text-slate-500 line-clamp-1 max-w-[200px] md:max-w-md">${escapeHtml(a.direccion || 'Sin dirección registrada')}</p>
                    </div>
                </div>
            </div>
            <div class="flex items-center gap-2 w-full sm:w-auto justify-end border-t sm:border-t-0 pt-4 sm:pt-0">
                ${!a.lat ? `<button data-gps="${a.id}" class="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all shadow-md shadow-indigo-100 active:scale-95">Fijar GPS</button>` : ''}
...
async function addAgencyLocal(data) {
...
async function updateAgencyLocal(agency) {
    const db = await dbPromise;
    await db.put('agencies', agency);
}

async function deleteAgencyLocal(id) {
    const db = await dbPromise;
...
async function init() {
    ...
    renderAgencies(cachedAgencies);  // ← DESCOMENTADO
}
...
form?.addEventListener('submit', async e => {
    e.preventDefault();
    const idReal = document.getElementById('idReal');
    const zona = document.getElementById('zona');
    const direccion = document.getElementById('direccion');
    
    await addAgencyLocal({
...
function applyFilters() {
    ...
    const order = orderSelect?.value || 'az';
    const state = stateFilter?.value || 'all';
    let filtered = cachedAgencies;
    if (q) {
        filtered = filtered.filter(a => a.idReal.toLowerCase().includes(q) || a.zona.toLowerCase().includes(q) || (a.direccion || '').toLowerCase().includes(q));
    }
...
list?.addEventListener('click', async e => {
    ...
    if (!btn) return;
    const id = btn.dataset.gps || btn.dataset.del || btn.dataset.edit;
    if (!id) return;
    ...
    if (btn.dataset.del) {
        const res = await Swal.fire({ title: '¿Eliminar agencia?', text: agency.idReal, icon: 'warning', showCancelButton: true, confirmButtonColor: '#ef4444', confirmButtonText: 'Eliminar' });
        if (res.isConfirmed) {
            await deleteAgencyLocal(id);
    ...
    if (btn.dataset.gps) {
        ...
        navigator.geolocation.getCurrentPosition(async pos => {
            ...
            await updateAgencyLocal(agency);
            Swal.fire({ icon: 'success', title: 'Ubicación asignada', timer: 1200, showConfirmButton: false });
            init();
    ...
    if (btn.dataset.edit) {
        const { value: formValues } = await Swal.fire({
            ...
            html: `<input id="swal-id" class="swal2-input" value="${agency.idReal}"><input id="swal-zona" class="swal2-input" value="${agency.zona}"><input id="swal-dir" class="swal2-input" value="${agency.direccion || ''}">`,
            showCancelButton: true,
            preConfirm: () => ({ idReal: document.getElementById('swal-id').value.trim(), zona: document.getElementById('swal-zona').value.trim(), direccion: document.getElementById('swal-dir').value.trim() })
        });
        if (formValues) {
            Object.assign(agency, formValues);
            await updateAgencyLocal(agency);
            Swal.fire({ icon: 'success', title: 'Agencia actualizada', timer: 1200, showConfirmButton: false });
            init();
        }
    }
});